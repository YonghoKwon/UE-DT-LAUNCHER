...
상용 PC는 MSI/RPM으로 런처와 업데이트 서비스를 함께 설치합니다. 개발·portable 검증은 폴더 배포도 지원합니다:
...
런처가 열리면 **업데이트 서비스 확인 → 배포 확인 → 설치 상태 확인**을 자동 수행합니다. 프로젝트가 하나면 선택 사이드바를 숨기고, 여러 개일 때만 검색/선택 UI를 표시합니다.

- **설치 상태** — 최신/업데이트 가능/설치 필요/오류
- **설치 버전** — 설치된 버전과 카탈로그 최신 버전을 나란히 표시. 초록=최신 상태, 주황=업데이트 필요
- **상태별 한 버튼** — 미설치면 `설치 후 실행`, 새 버전이 있으면 `업데이트 후 실행`, 최신 상태면 `실행`으로 자동 변경됩니다
- **진행 단계** — `배포 확인 → 다운로드 → 검증 → 설치 → 실행` 순서와 전체 진행률을 실시간으로 표시합니다
- **문제 해결** — 오류가 있을 때만 나타나며 서비스 재확인 → 상태 검사 → 파일 복구를 수행합니다. 백업 롤백은 마지막 수단으로 제안하고 실행 전 확인을 받습니다
- **설정 팝업** — 설치 버전/캐시·백업 용량 확인, 설치 폴더 열기, 로그 ZIP 저장. 직접 롤백은 개발자 화면에만 표시됩니다

일반 사용자에게는 Agent, IPC, Manifest, 내부 경로와 예외를 표시하지 않습니다. 오류가 나면 `다시 확인` 또는 `문제 해결`을 사용하고, 계속 실패하면 "로그 ZIP 저장"으로 만든 파일을 관리자에게 전달합니다.
