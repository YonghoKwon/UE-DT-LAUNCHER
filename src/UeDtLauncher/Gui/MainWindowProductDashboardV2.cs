...
public sealed partial class MainWindow : Window
{
    ...
    private TextBlock? _installStateCaption;
    ...
    private string LauncherBaseDir => AppContext.BaseDirectory;
    ...
    private Control BuildSidebar()
    {
        ...
        titleRow.Children.Add(T("프로젝트", 18, true));
        if (IsDeveloper)
        {
            var reloadButton = SmallButton("↻", (_, _) => { LoadConfigForUi(); BuildDashboard(); });
            Grid.SetColumn(reloadButton, 1);
            titleRow.Children.Add(reloadButton);
        }
        Grid.SetRow(titleRow, 0);
        sidebarGrid.Children.Add(titleRow);
        ...
        sidebarGrid.Children.Add(searchBox);
        ...
        sidebarGrid.Children.Add(projectScroll);
        ...
        sidebarGrid.Children.Add(bottom);

        var sidebar = Card(sidebarGrid, 14);
    ...
    private Control ProjectCard(ProjectUiConfig project)
    {
        ...
        var card = new Border
        {
            ...
            Background = B(IsDeveloper ? selected ? "#1E293B" : "#151E2A" : selected ? "#EFF6FF" : "#FFFFFF"),
        ...
        info.Children.Add(WrapText(project.DisplayName, 14, true, Fg()));
        info.Children.Add(WrapText(project.Status ?? ModeStatus(), 12, false, StatusBrush(project.Status)));
        info.Children.Add(WrapText(project.Technology ?? _config.TargetPlatform, 11, false, MutedBrush()));
    ...
    private Control Hero(double height)
    {
        ...
        overlay.Children.Add(new StackPanel
        {
            ...
            Children =
            {
                new TextBlock { Text = _selectedProject.DisplayName, FontSize = IsDeveloper ? 30 : 38, FontWeight = FontWeight.Bold, Foreground = Brushes.White, TextWrapping = TextWrapping.Wrap },
                new TextBlock { Text = IsDeveloper ? $"개발자 모드 · {_config.Environment} · {_config.Channel} · {_config.TargetPlatform}" : "안정화 최신 버전", FontSize = 16, Foreground = B("#93C5FD"), TextWrapping = TextWrapping.Wrap },
                new TextBlock { Text = _selectedProject.Description ?? "프로젝트를 최신 상태로 유지합니다.", FontSize = 13, Foreground = B("#D1D5DB"), TextWrapping = TextWrapping.Wrap }
            }
        });
        return overlay;
    }
    ...
    private Control StatusInfoTile()
    {
        ...
        _installStateText = WrapText(_installState, 17, true, StatusBrush(_installState));
        panel.Children.Add(_installStateText);
        _installStateCaption = WrapText(_installStateDetail, 12, false, MutedBrush());
    ...
    private Control GeneralActions()
    {
        ...
        var runButton = PrimaryButton("▶ 실행", (_, _) => _ = RunAsync(repair: false, launch: true), 132);
        Grid.SetRowSpan(runButton, 2);
        layout.Children.Add(runButton);
    ...
    {
        _storageText = WrapText(StorageSummary(), 13, false, Fg());
    ...
    private Control StatusPanel(bool showDeveloperLog)
    {
        ...
        _retryButton = SecondaryButton("다시 시도", (_, _) => _ = RunAsync(_lastRunRepair, _lastRunLaunch), 34);
        _retryButton.IsVisible = false;
        Grid.SetColumn(_retryButton, 1);
        header.Children.Add(_retryButton);
        var saveButton = SecondaryButton("로그 저장", (_, _) => SaveLog(), 34);
        ...
        _logBox = new TextBox
        {
            ...
            Height = showDeveloperLog ? 260 : 72,
    ...
    private Control InfoTile(string title, string value, string caption)
    {
        return Card(new StackPanel
        {
            ...
            {
                Muted(title, 13),
                WrapText(value, 17, true, Fg()),
                WrapText(caption, 12, false, MutedBrush())
            }
        }, 18);
    }
    ...
    private Control KeyValue(string key, string? value)
    {
        ...
        var valueBlock = WrapText(value ?? "-", 13, false, Fg());
    ...
    {
        if (_statusText is not null) _statusText.Text = statusText;
        _installState = "오류";
    ...
    private void UiProgress(string stage, string message, double? percent)
    {
        ...
        {
            if (_statusText is not null) _statusText.Text = FriendlyProgress(stage, message);
    ...
    {
        var path = Path.GetFullPath(Path.Combine(LauncherBaseDir, _selectedProject.InstallPath ?? _config.InstallDir));
    ...
    private void ClearCache()
    {
        try
        {
            var stagingDir = ResolveLauncherRelativePath(_config.StagingDir);
    ...
    private void SaveLog()
    {
        try
        {
            var logDir = Path.Combine(LauncherBaseDir, "logs");
    ...
    {
        var cache = DirectorySizeSafe(ResolveLauncherRelativePath(_config.StagingDir));
        var backup = DirectorySizeSafe(ResolveLauncherRelativePath(_config.BackupDir));
    ...
    private string GetConfigPathSafe()
    {
        try
        {
            if (_configPathBox is not null && !string.IsNullOrWhiteSpace(_configPathBox.Text))
            {
                return ResolveLauncherRelativePath(_configPathBox.Text.Trim());
            }
        }
        catch
        {
        }
        return Path.Combine(LauncherBaseDir, "launcher.config.json");
    }

    private string ResolveLauncherRelativePath(string path)
    {
        return Path.IsPathRooted(path) ? path : Path.Combine(LauncherBaseDir, path);
    }

    private string ModeStatus() => IsDeveloper ? "개발자 빌드" : "안정 버전";
    private TextBlock T(string text, double size, bool bold) => WrapText(text, size, bold, Fg());
    private TextBlock Muted(string text, double size) => WrapText(text, size, false, MutedBrush());
    private TextBlock WrapText(string text, double size, bool bold, IBrush foreground) => new() { Text = text, FontSize = size, FontWeight = bold ? FontWeight.SemiBold : FontWeight.Normal, Foreground = foreground, TextWrapping = TextWrapping.Wrap };
