...
public sealed partial class MainWindow : Window
{
    ...
    private string FriendlyError(Exception ex) { var m = ex.GetBaseException().Message; if (m.Contains("requestedVersion is required", StringComparison.OrdinalIgnoreCase)) return "exact 버전을 사용하려면 요청 버전을 입력해야 합니다."; if (m.Contains("403", StringComparison.OrdinalIgnoreCase) || m.Contains("Forbidden", StringComparison.OrdinalIgnoreCase)) return "이 네트워크(IP)에서는 이 프로젝트에 접근이 허용되지 않았습니다. 관리자에게 문의하세요."; if (m.Contains("No release in catalog matched", StringComparison.OrdinalIgnoreCase) || m.Contains("No allowed release", StringComparison.OrdinalIgnoreCase) || m.Contains("No release with version", StringComparison.OrdinalIgnoreCase)) return "현재 선택(프로젝트/환경/채널/플랫폼)으로 받을 수 있는 배포 버전이 없습니다."; if (m.Contains("No such host", StringComparison.OrdinalIgnoreCase) || m.Contains("actively refused", StringComparison.OrdinalIgnoreCase)) return "업데이트 서버에 연결할 수 없습니다. 네트워크와 서버 주소를 확인하세요."; return IsDeveloper ? m : "작업 중 문제가 발생했습니다. 잠시 후 다시 시도하거나 관리자에게 문의하세요."; }
